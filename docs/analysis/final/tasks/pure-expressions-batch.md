...
- `$function` and `$accumulator` are parser/runtime stubs in this Tier 1 backport. MongoDB executes user-provided JavaScript for these operators when server-side scripting is enabled. EloqDoc accepts the operator syntax but raises `server-side scripting disabled` at runtime instead of executing JavaScript. This is an intentional semantic difference for the Tier 1 compatibility scope; full server-side JavaScript execution remains future work.
