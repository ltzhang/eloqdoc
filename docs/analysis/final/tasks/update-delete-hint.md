...
- `update`/`delete` with valid `hint` (string or object) execute against the hinted index when applicable.
- Invalid hint (nonexistent index name, mismatched key pattern) returns `BadValue`.
