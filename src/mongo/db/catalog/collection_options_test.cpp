...
TEST(CollectionOptions, RejectsInvalidTimeSeriesOptions) {
    ASSERT_NOT_OK(CollectionOptions().parse(fromjson("{timeseries: {metaField: 'tags'}}")));
    ASSERT_NOT_OK(CollectionOptions().parse(fromjson("{timeseries: {timeField: 5}}")));
    ASSERT_NOT_OK(
